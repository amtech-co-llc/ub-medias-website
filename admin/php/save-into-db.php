...
if (!empty($titre) && !empty($categorie) && !empty($nom_du_redacteur) && !empty($description)) {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === 0) {
        ...
        if (move_uploaded_file($fileTmpPath, $outputFilePath)) {
            // Corrected line
            $sql = "INSERT INTO publication(`unique_id`,`titre`,`description`,`categorie`,`nom_du_redacteur`,`image`,`seo_url`,`post_likes`,`post_dislikes`,`post_views`,`post_comment`,`post_share`) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)";
            $query = $pdo->prepare($sql);
            $save = $query->execute([$unique_id, $titre, $description, $categorie, $nom_du_redacteur, $newFileName, $save_seo_url, '0', '0', '1', '0', '0']);
            if ($save) {
