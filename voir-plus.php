...
$sql_fetch = "SELECT* FROM publication WHERE `seo_url` = ?";
$query_fetch = $pdo->prepare($sql_fetch);
$query_fetch->execute([$slug]);

$result_fetch = $query_fetch->fetch();

if (count($result_fetch) > 0) {
    $sql = "UPDATE publication SET `post_views` = ? WHERE `seo_url` = ?";
    $query = $pdo->prepare($sql);
    $query->execute([intval($result_fetch['post_views']) + 1, $slug]);
} else {
    echo "no data available with this id: " . $id;
}

$sql_f = "SELECT * FROM publication WHERE `seo_url` = ?";
$query = $pdo->prepare($sql_f);
$query->execute([$slug]);
...
<head>
    ...
    <meta name="description" content="<?php echo $result['titre'] ?>">
    ...
    <div class="container">
        ...
        <div class="body-contents">
            ...
            <div class="contents-1">
                ...
                $sql = "SELECT * FROM publication WHERE `seo_url` = ? ORDER BY id DESC LIMIT 1";
                $query = $pdo->prepare($sql);
                $query->execute([$slug]);
                            ...
                            <div class="details">
                                ...
                                <div class="other-details">
                                    <div class="icons">
                                        ...
                                        <a href="./partager.php?id=<?php echo $row['id'] ?>" id="redir">
                ...
                <div class="other-cards">
                    ...
                    <h1>Actualités similaires</h1>

                    <div class="cards" id="items-container">
                        <!-- beginning of card1 -->
                        <?php

                        $sql = "SELECT * FROM publication WHERE `categorie` = '{$result['categorie']}' AND id != (SELECT MAX(id) FROM publication WHERE `categorie` = '{$result['categorie']}') ORDER BY RAND()";
                        $query = $pdo->prepare($sql);
                        $query->execute();
                                ...
                                <div class="card1">
                                    ...
                                    <div class="details">
                                        ...
                                        <div class="icons-activity">
                                            ...
                                            <a href="./comment.php?id=<?php echo $row['id'] ?>" id="redir">
                                                <div class="act"><i class="ri-chat-3-line"></i> <?php echo $row['post_comment'] ?></div>
                                            </a>
                                            <a href="./partager.php?id=<?php echo $row['id'] ?>" id="redir">
                                                <div class="act"><i class="ri-share-forward-line"></i> <?php echo $row['post_share'] ?></div>
                                            </a>
                                        </div>
                                    </div>
                                </div>
