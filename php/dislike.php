...
if (count($result) > 0) {
    ...
    if (intval($result['post_dislikes']) > 0 || intval($result['post_likes']) > 0) {
        $sql_likes = "UPDATE publication SET `post_dislikes` = ?, `post_views` = ? WHERE id = ?";
        $query_likes = $pdo->prepare($sql_likes);
        $query_likes->execute([intval($result['post_dislikes']) + 1, intval($result['post_views']) + 1, $post_id]);
    } else {
        $sql_likes2 = "UPDATE publication SET `post_dislikes` = ?, `post_views` = ? WHERE id = ?";
